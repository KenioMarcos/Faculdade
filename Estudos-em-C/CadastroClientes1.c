...
int opMenu, i, cod, op, limite, total = Max;
...
int atualizaMontante() // Função para atualizar o montante de um cliente especifico, mostra a lista e pede o código do cliente à ser atualizado;
{ 
    int cod;
    float montanteAtual;
    ...
    if (cadastro[i].ativo == 1)
    {
        ...
        else
        {
            ...
            printf("\nMontante atualizado para %.2f!", cadastro[cod].compraMes);
            system("clear");
            printf("\n1 - Atualizar outro?\n0 - Sair \n");
            scanf("%d", &opMenu);
            if (opMenu == 1)
